...
### Known / in progress
- macOS 27: menu-bar item positions are owned by the system now, so after upgrading, Hidden Bar's separator can end up somewhere unhelpful -- including inside the system's own overflow menu (`»`), where you cannot see it. ⌘-drag the separator once so that the icons you want hidden sit to its left. The longer-term managed-overflow redesign is tracked separately (#366).
